function varargout = df_do_cached_calc_checksum_known(function_name,the_checksum,varargin)
...
if strcmp(cached_dir,'null')
    ...
    if ~exist(cached_dir,'dir')
        disp(['Warning from function "do_cached_calc": I was told by function "df_dir_of_caches" that directory "' char(10) cached_dir '" is where I should save my data' ...
            ', but it looks like this directory doesn''t exist.'  char(10) ...
            'I am going to create this directory, assuming that you''re running the cache for the first time.  Evaluating...']);
        rec_make_dir(cached_dir);
    end
    initialize_dir_for_caching(cached_dir);
    ...
    if exist(the_filename,'file')
        try
            ...
            if loaded.cached_no_args >= nargout
                ...
                need_to_calculate = 0;
                [junk,found] = promote_cache(the_checksum,cached_dir);
                if ~found
